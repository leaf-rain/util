...
func varFunc(modelName, confName, lockName, onceName string) string {
	return fmt.Sprintf("\n var %s = new(%s) \nvar %s = new(sync.RWMutex)\nvar %s = new(sync.Once)\n", modelName, confName, lockName, onceName)
}
...
func getFunc(modelName, lockName, confName, onceName string) string {
	return fmt.Sprintf(`
func Get%s(jsPath string) %s {
	%s.Do(func() {
		%s.loadConfig(jsPath)
	})
	%s.Lock()
	defer %s.Unlock()
	return *%s
}`, confName, confName, onceName, modelName, lockName, lockName, modelName)
}

func loadFunc(modelName, lockName, confName, confPath string) string {
	return fmt.Sprintf(`
func (model %s) loadConfig(jsPath string) { 
	var fullPath =path.Join(jsPath,path.Base("%s"))
	...
	v.WatchConfig()
}`, confName, confPath, modelName, lockName, lockName, modelName, confName, modelName)
}
...
func (t *GoTarget) ExitJson(typeStr, valStr string) string {
	...
	var onceName = FirstLower(suffixOnce(t.ConfName))
	//return "package Generated \n type AutoGenerated " + valStr + "\n"
	return headFunc(t.PkgName) + varFunc(modelName, t.ConfName, lockName, onceName) +
		modelFunc(t.ConfName, valStr) + getFunc(modelName, lockName, t.ConfName, onceName) + loadFunc(modelName, lockName, t.ConfName, t.ConfPath)
}
